meta :prefpane do
  ...
  accepts_list_for :prefix, %w[~/Library/PreferencePanes /Library/PreferencePanes]
  accepts_list_for :extra_source
  accepts_list_for :provides, :name
  accepts_block_for :current_version do |path| nil end
  accepts_block_for :latest_version

  def prefpane_dir prefpane_name
    prefix.find {|prefpane_path|
      (prefpane_path.to_s / prefpane_name).glob.select {|entry|
        (entry / 'Contents/MacOS').exists?
      }.first
    }
  end

  def prefpane_name_match
    provides.first.to_s.sub(/\.prefpane$/, '*.prefPane')
  end
  ...
  template {
    ...
    met? {
      ...
      installed = prefpane_dir prefpane_name_match
    ...
    meet {
      process_sources {|archive|
        matches = Dir.glob("**/#{prefpane_name_match}").select {|entry|
          (entry / 'Contents/MacOS').exists? # must be an prefPane bundle itself
        }.reject {|entry|
